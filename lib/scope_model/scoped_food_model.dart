import 'dart:convert';

import 'package:foodie/models/food_model.dart';
import 'package:scoped_model/scoped_model.dart';
import 'package:http/http.dart' as http;
class FoodModel extends Model{
  bool _isLoading=false;
  List<Food> _foods=[];
  bool get isLoading{
    return _isLoading;
  }
  List<Food> get foods{
    return List.from(_foods);
  }
  int get foodsLength{
    return _foods.length;
  }
  Future<bool>  addFood(Food food)async{
    _isLoading=true;
    notifyListeners();
   try{

     final Map<String,dynamic> foodData={
       "title":food.title,
       "description":food.description,
       "category":food.category,
       "price":food.price,
       "discount":food.discount
     };
     final http.Response response =await http.post('https://newone-2ec23-default-rtdb.firebaseio.com/foods.json',
         body: json.encode(foodData));
     final Map<String , dynamic> responseData =json.decode(response.body);
     //  print(responseData["category"]);
     Food foodWithID=Food(
         id: responseData["name"],
         title:food.title,
         description:food.description,
         category:food.category,
         price:food.price,
         discount:food.discount
     );
      _isLoading=false;
     notifyListeners();
     _foods.add(foodWithID);
     //fetchFoods();
     return Future.value(true);
   }catch (e){
     _isLoading=false;
     notifyListeners();
     return Future.value(false);
   }

  }
  Future<bool> fetchFoods ()async{
    _isLoading=true;
    notifyListeners();
   try{
     final http.Response response =await http.get('https://newone-2ec23-default-rtdb.firebaseio.com/foods.json');
     final Map<String, dynamic> fetchData=json.decode(response.body);
     final List<Food> foodItems=[];
     fetchData.forEach((String id, dynamic foodData) {
       Food foodItem=Food(
         id: id,
         title: foodData["title"],
         category: foodData["category"],
         description: foodData["description"],
         price: double.parse(foodData["price"].toString()),
         discount: double.parse(foodData["discount"].toString()),

       );
       foodItems.add(foodItem);
     });
     _foods=foodItems;
     _isLoading=false;
     notifyListeners();
     return Future.value(true);
   }catch (e){
      _isLoading=false;
      return Future.value(false);
   }
  }
  Future<bool> updateFood(Map<String, dynamic> foodData,String foodId)async{
    _isLoading=true;
    notifyListeners();
    Food theUpdatedFood=getFoodById(foodId);
    int foodIndex=_foods.indexOf(theUpdatedFood);

    try{
      http.put('https://newone-2ec23-default-rtdb.firebaseio.com/foods/${foodId}.json', body: json.encode(foodData));
      Food updatedFood =Food(
        id: foodId,
        title: foodData['title'],
        category: foodData['category'],
        description: foodData['description'],
        price: foodData['price'],
        discount: foodData['discount']
      );
      _foods[foodIndex]=updatedFood;
      _isLoading=false ;
      notifyListeners();
      return Future.value(true);
    }catch(e){
       _isLoading = false;
       notifyListeners();
       return Future.value(false);
    }
  }
  Future<bool> deleteFood(String foodId)async{
    try{
     await http.delete('https://newone-2ec23-default-rtdb.firebaseio.com/foods/${foodId}.json');
       _foods.removeWhere((Food food) => food.id==foodId);
     _isLoading=false;
      notifyListeners();
       return Future.value(true);
    }catch(e){
      _isLoading=false;
      notifyListeners();
      return Future.value(false);
    }

  }
  Food getFoodById(String foodId){
    Food food;
    for(int i=0;i<_foods.length;i++){
      if(_foods[i].id==foodId){
        food =_foods[i];
         break;
      }
    }
    return food;
  }
}


